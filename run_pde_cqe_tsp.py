# Main entry: RBD clustering + reconstruction + CLI for PDE–CQE.
# Place alongside tsp_common.py / pde.py / cqe.py
#
# Examples:
#   python run_pde_cqe_tsp.py DataSet/bays29.tsp
#   python run_pde_cqe_tsp.py DataSet/bays29.tsp --cqe-mech-diag
...
def optimize_subcluster(cluster, cities, cluster_id=0, record_cqe_diagnostics=False):
    """
    Local PDE + CQE optimization for one resource-compatible subproblem.
    ...
    try:
        print(
            f"Optimizing subcluster {cluster_id} ({num_cities} cities)... "
        ...
        qga = HybridQuantumGA(
            ...
            use_two_opt=False,
            use_cqe=True,
            grover_diagnostics=bool(record_cqe_diagnostics),
            record_cqe_diagnostics=bool(record_cqe_diagnostics),
        )
...
def solve_tsp(cities, max_cluster_size=8, record_cqe_diagnostics=False):
    """
    Full PDE–CQE pipeline with resource-bounded decomposition.

    Returns:
        final_route, final_dist, cqe_diagnostics
    """
    n = len(cities)
    print(f"Starting optimization of {n} cities...")
    print(
    ...
    print(f"Final global distance: {final_dist:.2f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="PDE–CQE TSP runner"
    )
    parser.add_argument(
        "tsp_file",
        nargs="?",
    ...
    print("\nBest route (first 20 city IDs):")
    print(
        " -> ".join(map(str, [cities[i][0] for i in best_route[:20]])) + " -> ..."
    )
    print(f"Shortest total distance: {best_dist:.2f}")
    plot_solution(
        cities,
        best_route,
        title="TSP Solution (PDE–CQE)",
        instance_name=instance_name,
    )
    if cqe_diags and args.cqe_mech_diag and not args.no_cqe_mech_plot:
        plot_cqe_marked_set_probability(
            cqe_diags,
            instance_name=instance_name,
            max_events=max(3, int(args.cqe_mech_events)),
