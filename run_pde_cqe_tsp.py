# Main entry: RBD clustering + reconstruction + CLI for PDE / Full(+CQE).
...
def solve_tsp(cities, max_cluster_size=8, mode="full", record_cqe_diagnostics=False):
    """
    mode ∈ {"qde", "full"} for Quantum PDE / Full(+CQE).

    Returns:
        final_route, final_dist, cqe_diagnostics
    """
    if mode not in ("qde", "full"):
        raise ValueError(f"Unknown mode={mode}; expected qde / full")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="PDE–CQE TSP runner (Full PDE+CQE or PDE-only)"
    )
    parser.add_argument(
        "tsp_file",
        nargs="?",
    ...
    parser.add_argument(
        ...
        choices=["full", "qde"],
        default="full",
        help="full=PDE+CQE; qde=PDE only (no CQE)",
    )
    parser.add_argument(
        "--cqe-mech-events",
        type=int,
