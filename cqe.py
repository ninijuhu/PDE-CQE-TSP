...
class CQEMixin:
    ...
    def _format_prob(self, p):
        """Readable probability: fixed decimals when large enough, else scientific."""
        if p is None:
            return "N/A"
        p = float(p)
        if p >= 1e-4:
    ...
    def _print_grover_diagnostic(self, diag):
        ...
        print("Before CQE:")
        print(f"  Elite probability      = {self._format_prob(diag['p_elite_before'])}")
        print(f"  Target-set probability = {self._format_prob(diag['p_target_before'])}")
        print(f"  Elite fitness (route)  = {diag['elite_fitness_before']:.6f}")
        print("After CQE:")
        print(f"  Elite probability      = {self._format_prob(diag['p_elite_after'])}")
        print(f"  Target-set probability = {self._format_prob(diag['p_target_after'])}")
